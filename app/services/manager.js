...
const generateIndexSubtasks = (client, task) => {
  if (!task.transfer.indices || !task.transfer.indices.names) {
    log.info('No indices transferified in task');
    return Promise.resolve([]);
  }

  return getIndices(client, task.transfer.indices.names)
...
const generateTemplateSubtasks = (client, task) => {
  if (!task.transfer.indices || !task.transfer.indices.templates) {
    log.info('No templates transferified in task');
    return Promise.resolve([]);
  }

  return getTemplates(client, task.transfer.indices.templates)
...
const generateDocumentSubtasks = (client, task) => {
  ...
  return getIndices(client, task.transfer.documents.fromIndices).then((allIndices)=> {
    const filters = createFilterFunctions(task.transfer.filters);
...
const createFilterFunctions = (filterSpec) => {
  return _.reduce(filterSpec, (result, filter, name)=> {
    ...
    result[name] = (value) => filterFunction(value, filter.arguments);
    return result;
  }, {});
};

...
var start = (params)=> {
  ...
  transfer.loadMutators(path.join(__dirname, '../mutators/'));
  ...
  return ifStringProvided(params.indices, transfer.transferIndices).then(()=> {
    ...
    return ifStringProvided(path.join(__dirname, '../filters/indices.js'), manager.setIndexFilter);
  }).then(()=> {
    // TODO: find a way to do this better
    return ifStringProvided(path.join(__dirname, '../comparators/indices.js'), manager.setIndexComparator);
  }).then(()=> {
